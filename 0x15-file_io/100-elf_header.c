...
OAOAOA/**
OAOAOA * elf_type - print ELF type
 * @buffer: the ELF header
OAOAOA * @big_endian: endianness (big endian if non-zero)
 */
OAOAOAvoid elf_type(const unsigned char *buffer, int big_endian)
{
OAOAOA	char *type_table[5] = {
		"NONE (No file type)",
OAOAOA		"REL (Relocatable file)",
OAOAOA		"EXEC (Executable file)",
...
OAOAOA/**
 * _read - read from a file and print an error message upon failure
OAOAOA * @fd: the file descriptor to read from
 * @buf: the buffer to write to
 * @count: the number of bytes to read
 */
void _read(int fd, char *buf, size_t count)
{
	if (read(fd, buf, count) != -1)
OAOAOA		return;
OAOAOA	write(STDERR_FILENO, "Error: Can't read from file\n", 28);
OAOAOA	_close(fd);
OAOAOA	exit(98);
}
...
OAOAOA	if (buffer[EI_CLASS] == ELFCLASS64)
	{
		printf("ELF64\n");
OAOAOA		return (64);
OAOAOA	}
...
OAOAOA * @argc: the argument count
OAOAOA * @argv: the argument values
 *
...
/**
OAOAOA * _strncmp - compare two strings
 * @s1: the first string
OAOAOA * @s2: the second string
 * @n: the max number of bytes to compare
 *
OAOAOA * Return: 0 if the first n bytes of s1 and s2 are equal, otherwise non-zero
 */
int _strncmp(const char *s1, const char *s2, size_t n)
{
	for ( ; n && *s1 && *s2; --n, ++s1, ++s2)
...
void _read(int fd, char *buf, size_t count)
OAOAOA{
OAOAOA	if (read(fd, buf, count) != -1)
		return;
	write(STDERR_FILENO, "Error: Can't read from file\n", 28);
	_close(fd);
	exit(98);
